<script lang="ts">
    ...
    import { authQueryOptions, type AuthUser } from '$lib/api/auth';
    ...
    import { fetchSessions, revokeSession, type SessionEntry } from '$lib/api/auth';
    ...
    function handleLinkDiscord(): void {
        // TODO: implement discord auth flow
    }
...
<div class="flex h-full flex-col overflow-y-auto">
    <div class="mx-auto w-full max-w-2xl px-6 py-8">
        <h1 class="mb-1 text-lg font-semibold text-foreground">Account Settings</h1>
        <p class="mb-8 text-sm text-muted-foreground">Manage your profile and connected accounts</p>
            ...
            <section class="mb-8">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between rounded-lg border border-border bg-card p-4">
                        <div class="flex items-center gap-3">
                            <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-[#F40552]/10">
                                <Cfxre class="h-4 w-auto text-[#F40552]" />
                            </div>
                            <div>
                                <p class="text-sm font-medium text-foreground">Cfx.re</p>
                                {#if user.providers.cfx}
                                    <p class="text-xs text-muted-foreground">{user.providers.cfx.username}</p>
                                {:else}
                                    <p class="text-xs text-muted-foreground/50">Not connected</p>
                                {/if}
                            </div>
                        </div>
                        {#if user.providers.cfx}
                    ...
                    <div class="flex items-center justify-between rounded-lg border border-border bg-card p-4">
                        <div class="flex items-center gap-3">
                            <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-[#5865F2]/10">
                                <Discord class="h-4 w-4 text-[#5865F2]" />
                            </div>
                            <div>
                                <p class="text-sm font-medium text-foreground">Discord</p>
                                {#if user.providers.discord}
                                    <p class="text-xs text-muted-foreground">{user.providers.discord.username}</p>
                                {:else}
                                    <p class="text-xs text-muted-foreground/50">Not connected</p>
                                {/if}
                            </div>
                ...
                {:else}
                    <div class="space-y-2">
                        {#each sessions as session (session.id)}
                            <div class="flex items-center justify-between rounded-lg border bg-card p-3 {session.isCurrent ? 'border-primary/30' : 'border-border'}">
                                <div class="flex items-center gap-3">
